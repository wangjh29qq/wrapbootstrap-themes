$("#btn-restore-all").bind("click",function(){$("#workspace .btn").not("#btn-restore-all").each(function(){$(this).trigger("click")})});$(".icon-menu > ul > li > a > i").each(function(){var menuitem=$(this).parent();menuitem.draggable({cursor:"move",revert:"invalid",snap:".navbar-inner form.form-search"})});
...
$("#menuitem-filter").bind("keyup",function(e){var input=$("#menuitem-filter").val().trim().toLowerCase();if(e.keyCode==27){$("#menuitem-filter").val("");input=""}$(".icon-menu > ul > li:nth-child(n+4)").each(function(){if(input.length>0&&($(this).attr("keywords")||"").toLowerCase().indexOf(input)==-1&&($(this).find("a > span").html()||"").toLowerCase().indexOf(input)==-1)display="none";else display="block";$(this).css("display",display)})});
$(".icon-menu > ul > li > a").each(function(){$(this).bind("mouseover",function(){var sidebar_margin=$("#main_wrapper").css("margin-left");var sidebar_padding=$("#main_wrapper").css("padding-left");var width=$(this).parent().width()*1+sidebar_margin.replace("px","")*1+sidebar_padding.replace("px","")*1;if($("#main_wrapper").width()<=640)width=50;else if($("#main_wrapper").width()<=800)width-=4;$(this).next(".quick-widget, .sub-menus").each(function(){$(this).css("left",width)})})});load_fav_menus();
$("a[data-toggle=tooltip]").tooltip().click(function(e){e.preventDefault()});$("a[data-toggle=popover]").popover().click(function(e){e.preventDefault()});
...
function add_to_favlinks(menuitem){menuitem.attr("id",(new Date).getTime().toString());menuitem.addClass("btn").css({"left":0,"top":0,"margin-right":"4px"});menuitem.draggable({start:function(){$(this).remove();save_fav_menus()}});$.bootstrapGrowl("<img src='img/icons/heart.png' alt='' /> I'm persistent, you can drag again to remove me.",{align:"center"});$(".navbar-inner form.form-search").append(menuitem);var outer_html=$("<div></div>").append(menuitem.clone()).html();if(!localStorage.fav_menus_str||
localStorage.fav_menus_str.length==0)localStorage.fav_menus_str=outer_html;else localStorage.fav_menus_str+="@"+outer_html}function load_fav_menus(){if(localStorage.fav_menus_str&&localStorage.fav_menus_str.length>0){var array=localStorage.fav_menus_str.split("@");for(idx in array){var menuitem=$(array[idx]);menuitem.appendTo(".navbar-inner form.form-search");menuitem.draggable({start:function(){$(this).remove();save_fav_menus()}})}}}
function save_fav_menus(){var array=[];$(".navbar-inner form.form-search .ui-draggable").each(function(){var outer_html=$("<div></div>").append($(this).clone()).html();array.push(outer_html)});if(array.length>0)localStorage.fav_menus_str=array.join("@");else localStorage.fav_menus_str=undefined};